---
title: App object in Power Apps - Power Platform
tags: PowerFx Reference
createdAt: Mon Dec 29 2025 17:18:39 GMT+1100 (Australian Eastern Daylight Time)
updatedAt: Mon Dec 29 2025 17:18:59 GMT+1100 (Australian Eastern Daylight Time)
---
...
- The App object in Microsoft Power Apps has properties that identify the current screen and allow control over the app's behavior, such as the ActiveScreen property.
- The ConfirmExit and ConfirmExitMessage properties can be used to warn users before closing the app if there are unsaved changes, with ConfirmExit being a Boolean property and ConfirmExitMessage providing a custom message.
- The App object also supports named formulas, which allow for reusable formulas throughout the app, and user-defined functions, which enable writing custom functions with parameters and return values, similar to built-in functions in Microsoft Power Fx.
- The `Type` function is used to create an aggregate type for a library, allowing the return of a table of books from a function.
- User-defined functions can be created with the syntax `FunctionName( [ ParameterName1: ParameterType1 [ , ParameterName2: ParameterType2 ... ] ] ) : ReturnType = { Formula1 [ ; Formula2 ... ] }`, and can include behavior logic by wrapping the body in curly braces.
- The `OnError` function is used to take action when an error happens anywhere in the app, providing a global opportunity to intercept an error banner before being displayed to the end user.
- The `If` function checks the `Param` "admin-mode" to decide between `HomeScreen` and `AdminScreen`.
- The `If` function also checks if an attendee is a staff member to direct them to either `StaffPortal` or `HomeScreen`.
- The `IfError` function uses a `CustomConnector.APICall` to direct the app to either `ForestScreen` or `OceanScreen`, and displays `ErrorScreen` if the API call fails.




## Sources
- [website](https://learn.microsoft.com/en-us/power-platform/power-fx/reference/object-app)
**Applies to:**
...
![Screenshot of the App object selected in the Tree view pane. The properties pane is visible.](https://learn.microsoft.com/media/object-app/appobject.png)
...
Use the [Back](https://learn.microsoft.com/function-navigate) or [Navigate](https://learn.microsoft.com/function-navigate) function to switch the screen that's displayed.


The **BackEnabled** property changes how the app responds to the device back gesture (swipe or use the hardware back button on Android devices, or swipe from the left on iOS devices) when running in [Power Apps mobile](https://learn.microsoft.com/en-us/power-apps/mobile/run-powerapps-on-mobile). When enabled, the device back gesture goes back to the screen that was most recently shown, which is similar to the [Back](https://learn.microsoft.com/function-navigate#back) formula. When disabled, the device back gesture takes the user to the app list.
...
- Now, these properties can reference controls only on the first screen if the **Delayed load** preview feature is enabled (which it is by default for new apps). If you reference other screens, [Power Apps](https://app.getrecall.ai/item/33454c96-457e-4b59-8f14-b32cd08ac685) Studio doesn't show an error, but the published app doesn't open in [Power Apps](https://app.getrecall.ai/item/33454c96-457e-4b59-8f14-b32cd08ac685) Mobile or a browser. We're working to lift this limitation. In the meantime, turn off **Delayed load** in **Settings** > **Upcoming features** (under **Preview**).
...
When the user might have unsaved changes in the app, use this property to show a confirmation dialog box before exiting the app. Use a formula that checks variables and control properties (for example, the **Unsaved** property of the [Edit form](https://learn.microsoft.com/en-us/power-apps/maker/canvas-apps/controls/control-form-detail) control).

The confirmation dialog box appears in any situation where data can be lost, such as:

- Run the [Exit](https://learn.microsoft.com/function-exit) function.
...
- Run the [Launch](https://learn.microsoft.com/function-param) function with a *LaunchTarget* of **Self**.

- If the app runs in [Power Apps](https://app.getrecall.ai/item/33454c96-457e-4b59-8f14-b32cd08ac685) Mobile (iOS or Android):

- Swipe to switch to a different app in [Power Apps](https://app.getrecall.ai/item/33454c96-457e-4b59-8f14-b32cd08ac685) Mobile.

- Select the back button on an Android device.

- Run the [Launch](https://learn.microsoft.com/function-param) function to launch another canvas app.

The exact look of the confirmation dialog box can vary across devices and versions of [Power Apps](https://app.getrecall.ai/item/33454c96-457e-4b59-8f14-b32cd08ac685).

The confirmation dialog box doesn't show in [Power Apps](https://app.getrecall.ai/item/33454c96-457e-4b59-8f14-b32cd08ac685) Studio.
...
![Form-specific confirmation dialog box.](https://learn.microsoft.com/media/object-app/confirm-native-custom.png)

To export system-generated application logs to [Application Insights](https://learn.microsoft.com/en-us/power-apps/maker/canvas-apps/application-insights), set up the **Connection string** for your canvas app.

1. Open your app for [editing](https://learn.microsoft.com/en-us/power-apps/maker/canvas-apps/edit-app) in [Power Apps](https://app.getrecall.ai/item/33454c96-457e-4b59-8f14-b32cd08ac685) Studio.
...
In [Power Apps](https://app.getrecall.ai/item/33454c96-457e-4b59-8f14-b32cd08ac685), formulas determine the value of control properties. For example, to set the background color consistently across an app, you might set the **Fill** property for each to a common formula:
...
- **Named formulas is an Excel concept.** [Power Fx](https://app.getrecall.ai/item/d1b8596d-27f0-469b-b066-e8dd944787e6) uses Excel concepts where possible since so many people know Excel well. Named formulas are the equivalent of named cells and named formulas in Excel, managed with the Name Manager. They recalculate automatically like cells of a spreadsheet and control properties do.
...
- *ParameterType(s)* – Optional. The name of a type, either a built-in [data type name](https://learn.microsoft.com/data-types), a data source name, or a type defined with the **Type** function.
...
Each parameter and the output from the user define function must be typed. In this example, `SelectedGenre: Text` defines the first parameter to our function to be of type **Text** and `SelectedGenre` is the name of the parameter that is used in the body for the [Filter](https://learn.microsoft.com/function-filter-lookup). See [Data types](https://learn.microsoft.com/data-types) for the supported type names. The [Type](https://learn.microsoft.com/function-type) is used to create an aggregate type for our library, so that we can return a table of books from our function.

We defined `LibraryType` as a plural table of records type. If we want to pass a single book to a function, we can extract the type of the record for this table with the [RecordOf](https://learn.microsoft.com/function-type):
...
Record matching for function parameters is tighter than it is in other parts of [Power Fx](https://app.getrecall.ai/item/d1b8596d-27f0-469b-b066-e8dd944787e6). The fields of a record value must be a proper subset of the type definition and can't include additional fields. For example, `IsGenre( { Title: "My Book", Published: 2001 }, "Fiction" )` will result in an error.

Note, recursion isn't yet supported by user defined functions.

Named formulas and most user defined functions don't support behavior functions with side effects, such as [Set](https://learn.microsoft.com/function-set) or [Notify](https://learn.microsoft.com/function-showerror). In general, it's best to avoid updating state if you can, instead relying on functional programming patterns and allowing Power Fx to recalculate formulas as needed automatically. But, there are cases where it's unavoidable. To include behavior logic in a user defined function, wrap the body in curly braces:
...
- *ParameterType(s)* – Optional. The name of a type, either a built-in [data type name](https://learn.microsoft.com/data-types), a data source name, or a type defined with the [Type](https://learn.microsoft.com/function-type).
...
As with all [Power Fx](https://app.getrecall.ai/item/d1b8596d-27f0-469b-b066-e8dd944787e6) formulas, execution doesn't end when an error is encountered. After the [Error](https://learn.microsoft.com/function-iferror) has been called, the [If](https://learn.microsoft.com/function-if) prevents the changes to **Savings** and **Spent** from happening. The [IfError](https://learn.microsoft.com/function-iferror) can also be used to prevent further execution after an error. Even though it returns **Void**, the formula can still return an error if there's a problem.
...
Named formulas can be used with the [Type](https://learn.microsoft.com/function-type) function to create user defined types. Use `:=` instead of `=` to define a user defined type, for example `Book := Type( { Title: Text, Author: Text } )`. See the [Type](https://learn.microsoft.com/function-type) for more information and examples.

Use **OnError** to take action when an error happens anywhere in the app. It provides a global opportunity to intercept an error banner before being displayed to the end user. It can also be used to log an error with the [Trace](https://learn.microsoft.com/function-trace) or write to a database or web service.

In Canvas apps, the result of every formula evaluation is checked for an error. If an error is encountered, **OnError** is evaluated with the same **FirstError** and **AllErrors** scope variables that would have been used if the entire formula had been wrapped in an [IfError](https://learn.microsoft.com/function-iferror).


If **OnError** is empty, a default error banner shows the **FirstError.Message** of the error. Defining an **OnError** formula overrides this behavior, so the maker can handle error reporting as needed. You can request the default behavior in **OnError** by rethrowing the error with the [Error](https://learn.microsoft.com/function-iferror). Use the rethrowing approach if you want to filter out or handle some errors differently, but let others pass through.
...
![Label and slider control bound through the formula Label1.Text = 1/Slider1.Value.](https://learn.microsoft.com/media/object-app/onerror-noerror.png)

The slider defaults to 50. If the slider is moved to 0, **Label1** will show no value, and an error banner is shown:

![Slider control moved to 0, resulting in a division by zero error, and an error banner.](https://learn.microsoft.com/media/object-app/onerror-div0.png)
...
![App.OnError formula set to generate a Trace.](https://learn.microsoft.com/media/object-app/onerror-trace-formula.png)
...
![Slider control moved to 0, resulting in a division by zero error, but no error banner.](https://learn.microsoft.com/media/object-app/onerror-trace.png)
...
Using the **OnStart** property can cause performance problems when loading an app. We're creating alternatives for the top two reasons to use this property—caching data and setting up global variables. There's already an alternative for defining the first screen to show with [Navigate](https://learn.microsoft.com/function-navigate). Depending on your context, this property can be disabled by default. If you don't see it and need to use it, check the app's Advanced settings for a switch to enable it. You can also use the **OnVisible** property of a screen. By default, when the nonblocking **OnStart** rule is enabled, it lets the **OnStart** function run at the same time as other app rules. So, if variables referenced in other app rules are initialized within the **OnStart** function, they might not be fully initialized yet. Also, a screen can render and become interactive before either the **Screen.OnVisible** or **App.OnStart** functions finish running, especially if they take a long time to finish.


The **OnStart** property runs when the user starts the app. This property is often used to:

- Retrieve and cache data in collections by using the [Collect](https://learn.microsoft.com/function-clear-collect-clearcollect) function.

- Set up global variables by using the [Set](https://learn.microsoft.com/function-set) function.

This formula runs before the first screen appears. No screen is loaded, so you can't set context variables with the [UpdateContext](https://learn.microsoft.com/function-updatecontext) function. But you can pass context variables with the **Navigate** function.

After you change the **OnStart** property, test it by hovering over the **App** object in the **Tree view** pane, selecting the ellipsis (...), and then selecting **Run OnStart**. Unlike when the app loads for the first time, existing collections and variables are already set. To start with empty collections, use the [ClearCollect](https://learn.microsoft.com/function-clear-collect-clearcollect) function instead of the **Collect** function.

![App-item shortcut menu for Run OnStart](https://learn.microsoft.com/media/object-app/appobject-runonstart.png)

Note


- Using the [Navigate](https://learn.microsoft.com/function-navigate) function in the **OnStart** property is retired. Existing apps still work. For a limited time, you can enable it in the app settings (under **Retired**). But using **Navigate** this way can cause app load delays because it forces the system to finish running **OnStart** before showing the first screen. Use the **StartScreen** property instead to set the first screen displayed.

- The retired switch is off for apps created before March 2021 where you added **Navigate** to **OnStart** between March 2021 and now. When you edit these apps in [Power Apps](https://app.getrecall.ai/item/33454c96-457e-4b59-8f14-b32cd08ac685) Studio, you can see an error. Turn the retired switch on to clear this error.
...
- [Param](https://learn.microsoft.com/function-param) function to read parameters used to start the app.

- [User](https://learn.microsoft.com/function-user) function to read information about the current user.

- [LookUp](https://learn.microsoft.com/function-filter-lookup), [Filter](https://learn.microsoft.com/function-filter-lookup), [CountRows](https://learn.microsoft.com/function-table-counts), [Max](https://learn.microsoft.com/function-aggregates), and other functions that read from a data source.

- API calls through a connector. Make sure the call returns quickly.

- Signals such as [Connection](https://learn.microsoft.com/signals#connection), [Compass](https://learn.microsoft.com/signals#compass), and **App**.
...
![Navigate to StartScreen](https://learn.microsoft.com/media/object-app/appobject-runstartscreen.png)
...
Use the **StudioVersion** property to show or log the version of [Power Apps](https://app.getrecall.ai/item/33454c96-457e-4b59-8f14-b32cd08ac685) Studio used to publish an app. This property helps when you're debugging and checking that your app is republished with a recent version of [Power Apps](https://app.getrecall.ai/item/33454c96-457e-4b59-8f14-b32cd08ac685) Studio.
